...
<body>
    ...
    <div class="content">
        <h2 class="dashboard-title">Thêm Nhân Viên</h2>

        <form action="/SpringMVCPagination/nklnhanvien/save" method="post">
            <input type="hidden" name="nklIdNV" value="${command.nklIdNV}" />

            <div class="form-group">
                <label for="nklTenNV">Tên Nhân Viên:</label>
                <input type="text" class="form-control" id="nklTenNV" name="nklTenNV" value="${command.nklTenNV}" required>
            </div>
            ...
            <div class="form-group">
			    ...
			    <select class="form-control" id="nklChucVu" name="nklChucVu">
			        <c:forEach var="cv" items="${chucVuList}">
			            <option value="${cv}" ${cv == command.nklChucVu ? 'selected' : ''}>${cv.value}</option>
			        </c:forEach>
			    </select>
			</div>
            ...
            <div class="form-group">
                <label for="nklNgayVaoLam">Ngày Vào Làm:</label>
                <input type="date" class="form-control" id="nklNgayVaoLam" name="nklNgayVaoLam" value="${command.nklNgayVaoLam}" required>
            </div>

            <div class="btn-group">
                <button type="submit" class="btn btn-success"><i class="fas fa-save"></i> Lưu</button>
                <a href="/SpringMVCPagination/nklnhanvien/nklview" class="btn btn-secondary"><i class="fas fa-arrow-left"></i> Quay Lại</a>
            </div>
        </form>
    </div>
</body>
